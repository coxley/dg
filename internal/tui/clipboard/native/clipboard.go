...
func Write(t Format, buf []byte) <-chan struct{} {
	changed, err := WriteMany(Data{Format: t, Bytes: buf})
	if err != nil {
		if debug {
...
// WriteMany atomically replaces the clipboard with every supplied format.
func WriteMany(values ...Data) (<-chan struct{}, error) {
	lock.Lock()
	defer lock.Unlock()

	owned, err := prepareWriteMany(values)
	if err != nil {
		return nil, err
	}
	return writeMany(owned)
}

func prepareWriteMany(values []Data) ([]Data, error) {
	if len(values) == 0 {
		return nil, errors.New("clipboard write requires at least one format")
	}
	seen := make(map[Format]struct{}, len(values))
	owned := make([]Data, len(values))
	for i, value := range values {
		if _, ok := seen[value.Format]; ok {
		...
		owned[i] = Data{Format: value.Format, Bytes: slices.Clone(value.Bytes)}
		if value.Format == FmtImage {
			owned[i].Bytes = toPNG(owned[i].Bytes)
		}
	}
	return owned, nil
}
...
// Data pairs a clipboard format with its raw representation.
type Data struct {
	Format Format
	Bytes  []byte
}
