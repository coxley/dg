...
type wlSelectionData struct {
	mime  string
	bytes []byte
}

// wlWriteMany sets one selection with every supplied representation and serves
// paste requests until ownership is lost.
func wlWriteMany(values []Data) (<-chan struct{}, error) {
	...
	for _, value := range data {
		if err := w.request(sourceID, srcOpcodeOffer, wlEncodeString(value.mime)); err != nil {
			w.Close()
			return nil, err
		}
	}
...
func wlServeSend(w *wlConn, body []byte, data []wlSelectionData) {
	mime, _, err := wlString(body, 0)
	fd, ok := w.nextFd()
	if !ok {
		return
	}
