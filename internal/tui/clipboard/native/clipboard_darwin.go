...
var (
	...
	sel_dataUsingEncoding    = objc.RegisterName("dataUsingEncoding:")
)
...
func enumerateFormats() []Format {
	...
	n := int(types.Send(sel_count))
	out := make([]Format, 0, n)
	for i := 0; i < n; i++ {
		t := types.Send(sel_objectAtIndex, uintptr(i))
		if f, ok := darwinFormatFor(nsStringGo(t)); ok {
			out = append(out, f)
		}
	}
	return out
...
// nsStringGo converts an NSString to UTF-8 through an owned NSData value.
func nsStringGo(s objc.ID) string {
	if s == 0 {
		return ""
	}
	const nsUTF8StringEncoding = 4
	return string(nsdataBytes(s.Send(sel_dataUsingEncoding, nsUTF8StringEncoding)))
}
...
func clipboardWriteMany(values []Data) bool {
	...
	for _, value := range values {
		...
		var dataType objc.ID
		switch value.Format {
		case FmtText:
			dataType = objc.ID(_NSPasteboardTypeString)
		case FmtImage:
			dataType = objc.ID(_NSPasteboardTypePNG)
		default:
			mime, ok := formatMIME(value.Format)
			if !ok {
				return false
			}
			dataType = nsString(mime)
		}
		if pasteboard.Send(sel_setDataForType, data, dataType) == 0 {
			return false
		}
	}
	return true
