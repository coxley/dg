...
type x11SelectionData struct {
	target uint32
	bytes  []byte
}

// x11WriteMany takes ownership of CLIPBOARD and serves every representation
// until another writer replaces the selection.
func x11WriteMany(values []Data) (<-chan struct{}, error) {
	x, err := x11Connect()
	if err != nil {
		return nil, errUnavailable
	}
	...
	data := make([]x11SelectionData, len(values))
	for i, value := range values {
		name, err := x11TargetName(value.Format)
		if err != nil {
			x.Close()
			return nil, err
		}
		target, err := x.intern(name)
		if err != nil {
			x.Close()
			return nil, errUnavailable
		}
	...
	go func() {
		defer x.Close()
		x.serveSelection(sel, targets, data)
		done <- struct{}{}
		close(done)
	}()
	return done, nil
...
func x11TargetName(format Format) (string, error) {
	switch format {
	case FmtText:
...
func (x *x11conn) serveSelection(sel, targets uint32, data []x11SelectionData) {
	for {
		p, err := x11wire.NextEvent(x.r)
		if err != nil {
			return
		...
		case x11wire.EventSelectionRequest:
			...
			if req.Selection == sel {
				x.answerSelectionRequest(req, targets, data)
			}
		}
	}
}
...
func (x *x11conn) answerSelectionRequest(
	req x11wire.SelectionRequestEvent,
	targets uint32,
	data []x11SelectionData,
) {
	...
	if req.Target == targets {
		...
		atoms := make([]uint32, 1, len(data)+1)
		atoms[0] = targets
		for _, value := range data {
			atoms = append(atoms, value.target)
		}
		...
		for _, value := range data {
			if req.Target != value.target {
				continue
			}
			x.send(x11wire.ChangeProperty(
				req.Requestor,
				req.Property,
				value.target,
				8,
