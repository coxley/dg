...
- right drag on a node body resizes from the nearest corner; otherwise, right
  ...
  and aligned bend segments on selected edges move together while retaining
  their coordinates on the fixed axis
...
Ctrl-S opens the naming form for both drafts and named canvases. Named canvases
prefill their current section and name so the same action renames them.
